...
(function(){
  function show(){
    /* Remove any older/static release markers so exactly one version is visible. */
    Array.prototype.slice.call(document.querySelectorAll('*')).forEach(function(el){
      if(el.id==='segamap-version' || /^SegaMap v1\./.test((el.textContent||'').trim())) el.remove();
    });
    var el=document.createElement('div');
    el.id='segamap-version';
    el.textContent='SegaMap v1.7';
