/* SegaMap — Mauritius street & place search v1.4 */
(function(){
  var cache = {};
  var timers = {};
  var seq = {};
  ...
  function kind(item){
    var a = (item.addresstype || item.type || '').toLowerCase();
    var cat = (item.category || '').toLowerCase();
    if(a === 'road' || a === 'street' || a === 'pedestrian' || a === 'residential' || a === 'secondary' || a === 'primary' || a === 'tertiary') return 'RUE';
    if(['village','town','city','suburb','neighbourhood','locality','municipality','island'].indexOf(a) >= 0) return 'LIEU';
    if(cat === 'place' || cat === 'amenity' || cat === 'tourism' || cat === 'shop' || cat === 'leisure' || cat === 'building') return 'LIEU';
    return 'LIEU';
  }
  ...
  function online(q){
    ...
    var url = 'https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&namedetails=1&limit=8&countrycodes=mu&accept-language=fr&q=' + encodeURIComponent(q);
    return fetch(url, {headers:{'Accept':'application/json'}})
      ...
      .then(function(items){
        ...
        items.forEach(function(item, i){
          ...
          nodes[id].searchKind = kind(item);
          nodes[id].osmType = item.type || '';
          out.push(nodes[id]);
          if(allNodesList.indexOf(nodes[id]) === -1) allNodesList.push(nodes[id]);
        });
        cache[key] = out;
        return out;
      })
  ...
  function local(q){
    if(typeof searchNodes === 'function') return searchNodes(q) || [];
    return [];
  }
  ...
  function render(input, box, results){
    ...
    Array.prototype.forEach.call(box.querySelectorAll('[data-sega-search-id]'), function(el){
      el.addEventListener('click', function(){
        var id = el.getAttribute('data-sega-search-id');
        if(!nodes[id]) return;
        input.value = nodes[id].name;
  ...
  function attach(inputId, boxId){
    ...
    input.addEventListener('input', function(){
      ...
      var request = (seq[inputId] || 0) + 1;
      seq[inputId] = request;
      if(q.length < 2){ box.classList.remove('open'); return; }
      timers[inputId] = setTimeout(function(){
        Promise.all([Promise.resolve(local(q)), online(q)]).then(function(parts){
          if(seq[inputId] !== request || input.value.trim() !== q) return;
          render(input, box, merge(parts[0], parts[1]));
        });
      }, 180);
    }, true);
  }
  ...
  function boot(){
    attach('origin-input','origin-sugg');
    attach('dest-input','dest-sugg');
  }
